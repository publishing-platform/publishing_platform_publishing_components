...
RSpec.describe PublishingPlatformPublishingComponents::Presenters::ComponentWrapperHelper do
  describe "Component helper" do
    ...
    describe "classes" do
      it "accepts valid class names" do
        component_helper = described_class.new(classes: "gem-c-component app-c-component brand--thing brand__thing gem-print-force-link-styles alert alert-thing")
        expected = {
          class: "gem-c-component app-c-component brand--thing brand__thing gem-print-force-link-styles alert alert-thing",
        }
        expect(component_helper.all_attributes).to eql(expected)
      end
    ...
    describe "margins" do
      ...
      it "accepts a passed margin" do
        ...
        expect(helper.all_attributes[:class]).to eql("mb-5")
      end

      it "can set a margin, overriding a passed margin" do
        helper = described_class.new(margin_bottom: 5)
        helper.set_margin_bottom(0)
        expect(helper.all_attributes[:class]).to eql("mb-0")
      end
    end
  end
end
